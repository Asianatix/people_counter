...
class Detector(object):
    ...
    def _set_video_writer(self, video_path):
        ...
        self.video_output = cv2.VideoWriter(
            video_path, fourcc, fps, (self.im_width, self.im_height)
        )

    def __enter__(self):
        if self.args.tcp_ip_port is not None:
            self._set_tcp_client()
        self.vdo = VideoCapture(
            self.args.video_path,
            self.args.capture_buffer_length,
        ...
        if self.args.save_video_to:
            self._set_video_writer(
                "{}/0_{}_{}".format(
                    self.args.save_video_to, self.args.save_video_freq, self.vd_name
                )
            )
        ...
        for idx, each_im_ouputs in enumerate(batch_outs):
            ...
            if bbox_xcycwh is not None and bbox_xcycwh != []:
                ...
                if self.args.deep_sort:
                    ...
                    print(
                        "# of persons filter from  {} -> {}  after tracking".format(
                            persons_count_before_track, persons_count
                        )
                    )
                else:
    ...
    def _get_latest_frames(self, filter_policy=None):
        ...
        if not self.args.supress_verbose:
            print(
                "video_frame ids collected for processing: {}".format(
                    [x[0] for x in im]
                )
            )
        im = [x[1] for x in im]
        if len(im) > 1:
            ...
            if not self.args.supress_verbose:
                print(
                    "Only processing {} out of {} captured frames".format(
                        len(im), init_l
                    )
                )
        return flag, im
    ...
    def detect_video(self):
        ...
        while True:
            ...
            flag, imgs = self._get_latest_frames(
                filter_policy=self.args.buffer_filter_policy
            )
            if not flag:
                break
            ...
            im = imgs[-1]
            if persons_count > 0 and (
                self.args.display
                or self.args.save_video_to
                or (self.args.save_frames_to is not None)
            ):
            ...
            if self.args.save_video_to:
                ...
                if self.frame_count % self.args.save_video_freq == 0:
                    ...
                    self._set_video_writer(
                        "{}/{}_{}_{}".format(
                            self.args.save_video_to,
                            self.frame_count,
                            self.frame_count + self.args.save_video_freq,
                            self.vd_name,
                        )
                    )

            if self.args.save_frames_to is not None and persons_count > 0:
                frame_path = "{}/{}.jpg".format(
                    self.args.save_frames_to, self.frame_count
                )
                print(frame_path)
            ...
            if not self.args.supress_verbose:
                ...
                print(
                    "cap_frame:{} p_Frame:{} p_count:{} :  M_FPS:{} cap_FPS:{:.4f} Process_FPS:{} nn_time/avg:[{:.4f}/{:.4f}], frame_time/avg:[{:.4f}/{:.4f}]".format(
                        cap_f_count,
                        self.frame_count,
                        persons_count,
                        model_avg_fps,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path to video or path to rtsp stream",
    )
    parser.add_argument(
        "--real_time",
        action="store_true",
        help="If set true, Queue size of video capture buffer is set to <capture_buffer_length>.\
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Size of video capture buffer. This should be used in conjunction with real_time",
    )
    parser.add_argument(
        "--deepsort_checkpoint", type=str, default="deep_sort/deep/checkpoint/ckpt.t7"
    )
    parser.add_argument(
        "--proc_freq",
        type=int,
        default=1,
        help="Frequency at which nn model process  frames_list captured from video capture buffer.",
    )
    parser.add_argument(
        "--display", action="store_true", help="To display on cv window"
    )
    parser.add_argument(
        "--save_video_to", type=str, default=None, help="Save path to video"
    )
    parser.add_argument(
        "--save_video_freq",
        type=int,
        default=100000,
        help="Save video at this frequency of frames",
    )
    parser.add_argument("--use_cuda", type=str, default="True")
    parser.add_argument(
        "--supress_verbose", action="store_true", help="Supress print statements "
    )
    parser.add_argument(
        "--tcp_ip_port", type=str, help="IP:PORT of tcp server", default=None
    )
    parser.add_argument(
        "--save_frames_to",
        default=None,
        type=str,
        help="set this to path, to save predicted frames to be saved",
    )
    parser.add_argument(
        "--detectron_ckpt",
        help="Path to detectron checkpoint",
        default="/data/surveillance_weights/visdrone_t1/model_0111599.pth",
    )
    parser.add_argument(
        "--detectron_cfg",
        help="path to detectron cfg",
        default="/data/surveillance_weights/visdrone_t1/test.yaml",
    )
    parser.add_argument(
        "--buffer_filter_policy",
        type=str,
        default=None,
        help="This should be used only when captured buffer length > 3 frames.",
    )
    parser.add_argument(
        "--apply_batch_ensemble",
        action="store_true",
        help="Applies nms on predictions made on batch.\
                                                                            In conjunction with capture buffer length.",
    )
    parser.add_argument(
        "--deep_sort", action="store_true", help="Using deep sort to track people."
    )
